...
resource "terraform_data" "ansible" {
  ...
  provisioner "local-exec" {
    # TODO remove token
    command     = "ansible-playbook ${local.ansible_playbook} --tags 'apps' --extra-vars @group_vars/k3s_${local.cluster_name}.yml --extra-vars \"k3s_vault_token=${var.k3s_vault_token}\""
    working_dir = "../ansible"
    environment = {
      ANSIBLE_INVENTORY        = local.ansible_k3s_inventory
      ANSIBLE_CONFIG           = local.ansible_config
